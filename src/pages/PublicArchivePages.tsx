...
function RecordInlineDetail({ record }: { record: RecordEntry }) {
  ...
  useEffect(() => { let active=true; setHistory([]); services.records.history(record.id).then(value=>{if(active)setHistory(value);}).catch(()=>{if(active)setHistory([]);}); return()=>{active=false;}; }, [record.id]);
