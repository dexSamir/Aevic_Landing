...
export function RegisterPage() {
  ...
  if (receipt) return <div className="auth-form-shell auth-success"><CheckCircle2 size={42} /><span>Qeydiyyat sorğusu qəbul edildi</span><h1>Komandanız yoxlamaya göndərildi.</h1><RegistrationStatusPanel status={receipt.status} nextStep="Əvvəlcə e-poçtunuzdakı təsdiq keçidini açın. Komanda təsdiqindən sonra turnirə qoşula bilərsiniz." /><p>Şifrə və digər həssas məlumatlar brauzer yaddaşında saxlanılmadı.</p><div><Link className="button button--primary" to="/team"><span>Komanda panelini aç</span></Link><Link className="button button--ghost" to="/"><span>Ana səhifə</span></Link></div></div>;
