...
export function WrappedPage() {
  ...
  if (query.loading) return <main className="wrapped-shell"><div className="wrapped-loading"><LoadingSkeleton variant="dashboard" rows={6} /></div></main>;
  if (query.error && query.data === undefined) return <main className="wrapped-shell"><EmptyState heading="h1" title="Wrapped yüklənmədi" body="Public nəticə servisi hazırda cavab vermir." action={<Button onClick={query.refetch}>Yenidən cəhd et</Button>} /></main>;
  ...
  return <main className={`wrapped-shell ${paused ? 'is-paused' : ''}`} aria-label={`${summary.entity.name} AEVIC Wrapped ${summary.period.label}`}>
    ...
    <section className={`wrapped-story wrapped-story--${story.id}`}>
      ...
      <article key={story.id} className="wrapped-story__content" aria-live="polite" onPointerDown={(event) => { if (story.final || isWrappedInteractiveTarget(event.target) || event.button !== 0) return; pointerStart.current = { id: event.pointerId, x: event.clientX, y: event.clientY, time: performance.now() }; setHolding(true); event.currentTarget.setPointerCapture(event.pointerId); }} onPointerUp={pointerUp} onPointerCancel={() => { pointerStart.current = null; setHolding(false); }}>
        <div className="wrapped-facet" aria-hidden="true" /><span>{story.kicker}</span>{story.id === 'intro' && <TeamLogo name={summary.entity.name} src={summary.entity.logoUrl} size="lg" />}{story.value !== undefined && <strong className="wrapped-story__value">{story.value}</strong>}<h1>{story.title}</h1><p>{story.context}</p>{summary.historyIncomplete&&<p className="wrapped-history-disclosure">Yalnız sistemdə qeydə alınmış rəsmi nəticələr göstərilir. Əvvəlki tarixçə tam olmaya bilər.</p>}{story.id === 'intro' && showNavigationHint && <p className="wrapped-navigation-hint" role="status">← Sol/sağ toxun · üfüqi sürüşdür · ox düymələri →</p>}{story.id === 'victories' && <Trophy size={34} aria-hidden="true" />}
