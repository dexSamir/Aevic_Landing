...
describe('AEVIC reset email',()=>{
 it('provides branded HTML and a plain-text alternative without token-bearing previews or assets',()=>{
  ...
  expect(mail.html).toContain('#0a0a0c');expect(mail.html).toContain('#f3c450');expect(mail.html).toContain('30 dəqiqə');
  ...
  expect(mail.html).toContain('role="presentation"');expect(mail.html).toContain('max-width:560px');
 });
