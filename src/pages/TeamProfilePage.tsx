...
import { Button, Input, LoadingSkeleton, PageHeader, Textarea, Toast } from '../components/common/primitives';
    ...
    <div className="team-profile-workspace">
      <div className="team-profile-controls">
        <section className="settings-panel"><header><div><span>KOMANDA KİMLİYİ</span><h2>Komanda təqdimatı</h2><p>Komandanızın ictimai kimliyini yeniləyin və dəyişiklikləri saxlayın.</p></div></header><div className="settings-section">
          <div className="form-grid"><Input label="Komanda adı" value={draft.name} onChange={event => { setIdentityDirty(true); setDraft(value => ({ ...value, name: event.target.value })); }} /><Input disabled hint="Bu məlumatın saxlanması hazırda dəstəklənmir." label="Qısa teq" value={draft.tag ?? ''} maxLength={12} onChange={event => { setIdentityDirty(true); setDraft(value => ({ ...value, tag: event.target.value })); }} /></div>
          <div className="form-grid"><Input disabled hint="Bu məlumatın saxlanması hazırda dəstəklənmir." label="Ölkə" value={draft.country ?? ''} onChange={event => { setIdentityDirty(true); setDraft(value => ({ ...value, country: event.target.value })); }} /><Input disabled hint="Bu məlumatın saxlanması hazırda dəstəklənmir." label="Qurulduğu tarix" type="date" value={draft.foundedAt?.slice(0, 10) ?? ''} onChange={event => { setIdentityDirty(true); setDraft(value => ({ ...value, foundedAt: event.target.value || undefined })); }} /></div>
          <Textarea disabled label="Komanda haqqında" value={draft.description ?? ''} rows={4} onChange={event => { setIdentityDirty(true); setDraft(value => ({ ...value, description: event.target.value })); }} />
          <Input disabled hint="Bu məlumatın saxlanması hazırda dəstəklənmir." label="Banner təsviri" value={draft.bannerAlt ?? ''} onChange={event => { setIdentityDirty(true); setDraft(value => ({ ...value, bannerAlt: event.target.value })); }} />
          <Button variant="secondary" onClick={() => setShowMedia(true)} disabled={showMedia} aria-expanded={showMedia}>{showMedia ? 'Media seçimləri açıqdır' : 'Loqonu idarə et'}</Button>
          {showMedia && <Suspense fallback={<LoadingSkeleton variant="media"/>}><MediaPreview teamId={currentTeam.id} onPreview={(type, url) => setDraft(value => ({ ...value, [type === 'logo' ? 'logoUrl' : 'bannerUrl']: url }))} /></Suspense>}
