...
import type { Team, Tournament, TeamTournamentResult, MatchHistoryEntry, MatchScheduleItem, PublicTeamSummary, PublicTeamProfile, TeamPlatformSnapshot, Notification, AdminMessage, ResultDispute, RosterChangeRequest, RoundResult, TeamComparisonRecord } from '../../src/types/domain';
import { summarizeMapPerformance, deriveTeamForm } from '../../src/utils/competitionAnalytics';

import { achievements,organizations } from './identity';

type Row = Record<string, unknown>;
// Column grants intentionally exclude review notes, even for public rows.
const projections: Record<string, string> = {
 teams: 'id,name,slug,tag,logo_url,banner_url,banner_alt,description,country,founded_at,game_key,approval_status,verification_level,archived_at,created_at,updated_at,legacy_history_incomplete',
...
export class Repository {
 ...
 async standingSnapshots(tournamentId:string):Promise<import('../../src/types/domain').LeaderboardSnapshot[]> {
  ...
  const [matches,results]=await Promise.all([this.rows('matches'),this.results()]);
  const published=matches.filter(m=>m.tournament_id===tournamentId&&m.published_at).sort((a,b)=>s(a,'published_at').localeCompare(s(b,'published_at'))||s(a,'id').localeCompare(s(b,'id')));
  const ids=new Set<string>();
  return published.map(match=>{ids.add(s(match,'id'));return {id:s(match,'id'),tournamentId,publishedAt:s(match,'published_at'),standings:rankResults(results.filter(r=>r.tournamentId===tournamentId&&ids.has(r.roundId))).map(r=>({teamId:r.teamId,rank:r.placement,totalPoints:r.totalPoints}))};});
 }
 ...
 async comparisons():Promise<TeamComparisonRecord[]> {
  ...
  return teams.filter(t=>t.approvalStatus==='approved'&&!t.archivedAt).map(team=>{
   ...
   return {teamId:team.id,teamName:team.name,legacyHistoryIncomplete:team.legacyHistoryIncomplete,matches:rows.length,finishes:rows.reduce((s,r)=>s+r.finishes,0),wwcd:rows.filter(r=>r.placement===1).length,podiums:rows.filter(r=>r.placement<=3).length,
    ...
    championships:countChampionships(team.id,standings,tournaments)};
  });
 }
 async profile(slug:string):Promise<PublicTeamProfile> {
  ...
  return{team,organization:team.organizationId?(await organizations(this)).find(o=>o.id===team.organizationId):undefined,achievements:badges,featuredAchievementIds:badges.filter(a=>a.featured).map(a=>a.id),legacy,recentResults,career,recentMatches:history,upcomingMatch:schedule.find(m=>m.status!=='completed'&&registrations.some(r=>r.team_id===team.id&&r.tournament_id===m.tournamentId&&r.status==='confirmed')),form:deriveTeamForm(history),mapSpecialization:summarizeMapPerformance(team.id,history)};
 }
 async notifications(userId:string):Promise<Notification[]> {return(await this.rows('notifications')).filter(r=>r.recipient_id===userId).map(notification).sort((a,b)=>b.createdAt.localeCompare(a.createdAt));}
 async messages(teamId?:string):Promise<AdminMessage[]> {return(await this.rows('messages')).filter(r=>!teamId||!r.team_id||r.team_id===teamId).map(r=>({...notification(r),audience:r.team_id?'team' as const:'all' as const})).sort((a,b)=>b.createdAt.localeCompare(a.createdAt));}
 ...
 async teamSnapshot(teamId:string,userId:string):Promise<TeamPlatformSnapshot> {
  const [team,tournaments,registrations,checkins,schedule,history,leaderboard,notifications,messages,teams,matches]=await Promise.all([this.team(teamId,true),this.tournaments(),this.rows('tournament_registrations'),this.rows('check_ins'),this.schedule(),this.history(teamId),this.standings(),this.notifications(userId),this.messages(teamId),this.teams(),this.rows('matches')]);
...
function deriveCareerSummary(teamId:string,matches:MatchHistoryEntry[],championships:number):import('../../src/types/domain').CareerSummaryData {
 return {teamId,scopeLabel:'Rəsmi nəticələr',metrics:[
 {key:'championships',label:'Çempionluq',value:championships,description:'Tamamlanmış turnirdə birinci yer'},
...
/** The same official ranking and tie breaks power current and cumulative standings. */
export function rankResults(results:RoundResult[]):TeamTournamentResult[] {
 const grouped=new Map<string,TeamTournamentResult>();
...
export function countChampionships(teamId:string,standings:TeamTournamentResult[],tournaments:Tournament[]) { return standings.filter(r=>r.teamId===teamId&&r.placement===1&&tournaments.some(t=>t.id===r.tournamentId&&t.status==='completed')).length; }
