...
export function LegacyMatchRedirect() {
  ...
  const query = usePlatformQuery({ scope:'public', key: `match:${matchId}`, query: () => services.publicMatches.get(matchId), staleTime: queryPolicy.publicCompetition, retry: 0 });
...
export function SeasonArchivePage() {
  const query = usePlatformQuery({ scope:'public', key: 'archive', query: () => services.archive.seasons(), staleTime: queryPolicy.historical, retry: 0 });
