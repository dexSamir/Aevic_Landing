...
export function RegisterPage() {
  ...
  const checkPlayer = async (index: number) => {
    const uid=draft.players[index].uid;if(!/^\d{5,20}$/.test(uid))return false;
    setPlayerChecks(current=>({...current,[index]:{state:'checking'}}));
    try{const result=await services.registration.validatePlayer(uid);setPlayerChecks(current=>({...current,[index]:{state:result.eligible?'eligible':'unavailable'}}));return result.eligible;}
    catch{setPlayerChecks(current=>({...current,[index]:{state:'error'}}));return true;}
  };
  const validateRosterPlayers = async () => {if(Object.keys(getStepErrors(3,draft,password,confirmation,playerChecks)).length)return false;return (await Promise.all(draft.players.map((p,i)=>p.ign?checkPlayer(i):Promise.resolve(true)))).every(Boolean);};
  ...
  const renderPlayerEditor = (player: RegistrationPlayerDraft, index: number) => {
    ...
    return <article className={`roster-player ${openPlayer === index ? 'is-open' : ''} ${complete ? 'is-complete' : ''} ${error ? 'has-error' : ''}`} key={index}>
      ...
      {openPlayer === index && <div id={panelId} role="region" aria-labelledby={triggerId} className="roster-player__fields">
        ...
        <Input id={`player-${index}-uid`} hint="Oyundakı profilinizdən PUBG ID-ni daxil edin." label="PUBG ID" placeholder="51234567890" value={player.uid} onChange={(event) => { updatePlayer(index, { uid: normalizePubgId(event.target.value) }); setPlayerChecks((current) => ({ ...current, [index]: { state: 'idle' } })); setPlayerLookups((current) => ({ ...current, [index]: undefined })); }} onBlur={() => { touch(`player-${index}-uid`); void checkPlayer(index); }} error={uidError} optional={optional} required={!optional} />
        <p className="player-check-status" role="status" style={{minHeight:24,margin:0,gridColumn:'1 / -1'}}>{playerChecks[index]?.state === 'checking' ? 'Oyunçu yoxlanılır…' : playerChecks[index]?.state === 'unavailable' ? 'Bu PUBG ID başqa komandada qeydiyyatdadır.' : ''}</p>
