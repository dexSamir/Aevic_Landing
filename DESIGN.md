...
The authenticated Team Panel extends this authority through **Operations Field**: a focused warm-graphite workspace for competition operations. It preserves the AEVIC gold, purple, Orbitron, and Raleway vocabulary while replacing undifferentiated black panels with a deliberate opaque surface hierarchy. The current public team dossier and tournament pages are the visual reference for this subsystem; the remaining `/team` routes inherit its chrome, density, control language, and next-action-first logic rather than becoming independent visual concepts.
...
Logo frequency is contextual: public headers carry the compact mark, one primary hero may carry the larger identity moment, and the footer may repeat the compact brand for orientation. Protected desktop routes rely on the sidebar identity; below 1180px the compact official mark sits at the left of the product topbar; notifications, account and the drawer control stay at the right. Authentication and sharecard uses remain selective. Decorative watermarks and unnecessary duplicate large marks are not part of the system.
...
The `/team` overview is a captain-first run sheet, not a KPI-card dashboard. Its durable order is **What Now → Room / Next Match / Important Update / Map → Official Statistics → Recent Form → Active Competition → Updates**. The first viewport contains compact team identity, one required action, the next match, and the latest meaningful change. Participation, room release, match timing, announcements, and readiness continue to come from the real Team services; authoritative open check-in still uses the existing confirmation and service update flow.

The overview does not reintroduce KPI cards, quick-link grids, a duplicated command center, separate tournament briefs, standalone schedule cards, or the old readiness cards. Readiness is one compact end-of-run ledger. The complete Team workspace shares the public condensed display hierarchy, grouped navigation and editorial ledgers. Public profile management lives at `/team/profile`, while `/team/settings` owns service-backed preferences and `/team/settings/managers` owns authority and guarded destructive actions. Career, map statistics and Wrapped start at `/team/career`; history, comparison and Share Studio retain their existing URLs.
...
### Login / Register — Tournament Authentication

The `.auth-identity` scope in `src/styles/auth.css` gives Login and Register one restrained tournament identity while preserving the existing navbar structure, navigation items, routes, validation, and form behavior. These auth-specific treatments do not replace the public navigation or Team workspace systems.

- **Artwork and depth:** use the exact supplied PNGs, `src/assets/official/login-background.png` and `src/assets/official/register-background.png`, as full-viewport cover backgrounds over `#050506`. Use localized left-to-right readability gradients, preserving original image brightness on the right. Set distinct focal points for large desktop, standard desktop, tablet, mobile, and very small mobile. Keep the trophy visible; do not add glass, neon, bright gradients, or decorative frames.
- **Composition:** Login places its heading above a 460px maximum-width card in the left visual zone. Register places a larger left-aligned heading above a light four-step progress indicator, then a form with a simplified preview on the right. The compact public-style preview adds captain identity at Step 2, roster at Step 3, and truthful draft/readiness information at Step 4; contact details remain private. Desktop auth occupies one viewport with the shared footer below it; compact registration steps fit where possible; excess content scrolls internally without visible scrollbar chrome while actions stay visible. Below 1024px, Register stacks the form before the preview with natural scrolling; below 768px Login centers.
- **Cards and spacing:** use 20px corners, 22–24px form padding, and quiet one-pixel gold borders. Login uses `rgba(5,5,6,.86)` with `rgba(243,196,80,.14)` borders; Register form and preview use `rgba(5,5,6,.88)` with `rgba(243,196,80,.10)` borders. Form padding reduces to 24px on mobile and 20px at the narrowest breakpoint. No backdrop blur is required.
- **Shared controls:** inputs are 48px high, `#101012`, with 9px corners and `rgba(255,255,255,.08)` borders; leading icons stay quiet. Focus uses a gold border and a subtle gold ring. Primary buttons use `#F3C450` with `#090909` text, 9px corners, and a brighter `#f8d478` hover without movement or glow. Secondary navigation actions are gold and retain visible keyboard focus.
- **Editing and review:** team logo editing uses a local square canvas in a keyboard-accessible dialog, exports transparent PNG only on confirmation, and keeps original uploads off the network. Final review groups team identity, private captain contacts, roster, and regulations with independent edit actions. Agreement checkbox and regulations link are separate interactive elements. Conditional team-name status is outside the stable name/tag grid.
- **Typography and accents:** retain the shared display/body families, white headings, muted descriptions, and the `// AEVIC HESABI` label. Purple `#7A2FFF` appears only as one small label line on Login or one small motto line on Register. Keep entrance motion to the existing subtle route transition and honor reduced motion.
