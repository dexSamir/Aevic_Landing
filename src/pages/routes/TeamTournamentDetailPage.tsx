...
export function TeamTournamentDetailPage() {
  ...
  const terminal = ['completed','cancelled'].includes(activeTournament.status);
  const canWithdraw = !withdrawn && ['pending','confirmed','waitlisted'].includes(participation.status) && !['ongoing','completed','cancelled'].includes(activeTournament.status) && Date.now() < Date.parse(activeTournament.startsAt);
  const roomAvailability = withdrawn || context.participation.status === 'withdrawn' ? undefined : terminal ? undefined : room ?? context.room;
