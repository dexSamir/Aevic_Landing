...
export function PublicTeamDetail({ team, profile }: { team: Team | PublicTeamSummary; profile?: PublicTeamProfile }) {
  ...
  return <><article className="public-team-detail">
    ...
    <div className="public-team-width public-team-content">{team.legacyHistoryIncomplete&&<p role="status" className="public-team-note">Komanda mövcud qeydiyyat məlumatları ilə göstərilir. Tam yarış tarixçəsi əlçatan deyil; boş nəticələr əvvəlki fəaliyyətin yoxluğu demək deyil.</p>}
      ...
      <section id="roster" className="public-team-section"><h2>Aktiv heyət</h2>{roster.length ? <div className="public-team-roster">{roster.map(player => <article key={player.id}><div className="public-team-player-portrait" aria-hidden="true"><span>{player.ign.slice(0, 2).toUpperCase()}</span><Crosshair />{player.photoUrl && <img src={player.photoUrl} alt="" loading="lazy" referrerPolicy="no-referrer" onError={event => { event.currentTarget.hidden = true; }} />}</div>{player.role === 'captain' && <Crown className="public-team-captain" size={20} aria-label="Kapitan" />}<div className="public-team-player-copy">{player.role === 'captain' ? <Crown size={20} /> : <CirclePlus size={18} />}<div><h3>{player.ign}</h3><p>{player.role === 'captain' ? 'Kapitan' : player.role === 'starter' ? 'Əsas heyət' : 'Əvəzedici'}</p></div></div></article>)}</div> : <p className="public-team-empty">İctimai heyət məlumatı hələ dərc edilməyib.</p>}</section>
