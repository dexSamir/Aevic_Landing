...
export function TeamGovernancePage() {
  ...
  const backendReady = !serviceCapabilities.mockPreview;
  const invite = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (!canManage || !backendReady || loading) return;
  ...
  const allowed = backendReady && (decision === 'leave' ? Boolean(viewer) && !isOwner : decision === 'transfer' ? isOwner && serviceCapabilities.ownershipTransfer : decision === 'archive' ? isOwner : canManage);
  const confirm = async () => {
    ...
    try {
      ...
      setDecision(undefined); setNotice('Dəyişiklik təsdiqləndi.'); await load();
