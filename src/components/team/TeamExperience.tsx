...
export function PerformanceSummary({ result, recentMatches }: { result: TeamTournamentResult; recentMatches: MatchHistoryEntry[] }) {
  ...
  return <section className="performance-summary" aria-labelledby="performance-summary-title"><header><div><span>Dərc edilmiş rəsmi nəticələr</span><h2 id="performance-summary-title">Performans icmalı</h2></div><TrendingUp size={21} /></header><div className="performance-summary__lead"><strong>#{String(result.placement).padStart(2, '0')}</strong><span>Turnirdə yerləşmə</span></div><dl><div><dt>Matç</dt><dd>{result.matches}</dd></div><div><dt>WWCD</dt><dd>{result.wwcd}</dd></div><div><dt>Kill</dt><dd>{result.finishes}</dd></div><div><dt>Xal</dt><dd>{result.totalPoints}</dd></div></dl><footer><Swords size={17} /><span>Son {recentMatches.length} dərc edilmiş raund: <strong>{recentPoints} xal · {recentFinishes} kill</strong></span></footer></section>;
}
