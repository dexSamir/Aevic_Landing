...
export function TeamTournamentDetailPage() {
  ...
  useEffect(() => { setRoom(undefined); setRevealed(false); setRoomError(''); setCheckedIn(false); setCheckInOpen(false); }, [tournamentId]);
  useEffect(() => {
  ...
  return <><PageHeader eyebrow="Turnir əməliyyatları" title={activeTournament.name} description="Bu günün iştirak, vaxt və otaq xətti." actions={<><Link className="button button--secondary" to={`/tournaments/${activeTournament.id}`}><span>Public turnir</span></Link>{tournamentMatches[0] && <Link className="button button--secondary" to={`/tournaments/${activeTournament.id}#matches`} state={{ roundId: tournamentMatches[0].id }}><span>İlk raund</span><ArrowRight size={16} /></Link>}</>} />
    ...
    {context.checkIn && <section className="tournament-check-in" aria-label="Check-in əməliyyatı"><div><span>CHECK-IN</span><strong>{checkedIn || context.checkIn.status === 'checked-in' ? 'Check-in tamamlandı' : context.checkIn.status === 'open' ? 'İştirakınızı indi təsdiqləyin' : context.checkIn.status === 'missed' ? 'Check-in pəncərəsi bağlanıb' : 'Check-in pəncərəsi gözlənilir'}</strong></div>{!checkedIn && context.checkIn.status === 'open' && <Button onClick={() => setCheckInOpen(true)}>Check-in et</Button>}</section>}
    <ConfirmDialog open={checkInOpen} title="Check-in-i təsdiqlə" body={`${currentTeam.name} komandasının iştirak etməyə hazır olduğunu təsdiqləyirsiniz.`} confirmLabel="Bəli, hazırıq" tone="primary" onClose={() => setCheckInOpen(false)} onConfirm={async () => { await services.teams.checkIn(activeTournament.id); setCheckedIn(true); setCheckInOpen(false); }} />
