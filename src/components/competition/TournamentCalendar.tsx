import { ArrowRight, CalendarClock, Check, ChevronLeft, ChevronRight, LockKeyhole, Radio, Clock3, Users, Layers3 } from 'lucide-react';
import { useEffect, useMemo, useRef, useState, type KeyboardEvent } from 'react';
import { Link } from 'react-router-dom';
import { competitionNow, serviceCapabilities } from '../../services';
...
export function TournamentCalendar({ tournaments, compact = false, overview = false, planning = false, onDateSelect, currentTime, selection, onSelectionChange, participationVersion = 0, onParticipationChange }: { tournaments: Tournament[]; compact?: boolean; overview?: boolean; planning?: boolean; onDateSelect?: (date: string) => void; currentTime?: Date; selection?: { date: string; tournamentId: string }; onSelectionChange?: (selection: { date: string; tournamentId: string }) => void; participationVersion?: number; onParticipationChange?: (id: string) => void }) {
  ...
  return <section ref={calendarRef} className={`tournament-calendar ${compact ? 'tournament-calendar--compact' : ''} ${overview ? 'tournament-calendar--overview' : ''}`} aria-label="AEVIC turnir təqvimi">
    <div className="tournament-calendar__picker">
      ...
      {planning ? <div className="planning-legend" aria-label="Turnir statusları"><span><i />Qeydiyyat açıq</span><span><i />Canlı</span><span><i />Planlaşdırılıb</span><span><i />Tamamlanıb</span></div> : <div className="tournament-calendar__legend" aria-label="Turnir statusları"><span><CalendarClock size={13} />Açıq</span><span><LockKeyhole size={13} />Bağlı</span><span><Radio size={13} />Canlı</span><span><Check size={13} />Bitib</span></div>}
    </div>
