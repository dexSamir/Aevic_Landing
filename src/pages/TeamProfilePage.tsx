  ...
  return <><PageHeader eyebrow="// KOMANDANIN İCTİMAİ ÜZÜ" title="Public profil" description="Tamaşaçıların gördüyü kimlik, heyət və sosial keçidlər." actions={<Link className="button button--secondary" to={profilePath}>Public profili aç <ExternalLink size={17} /></Link>} />
    <div className="team-profile-workspace">
      <div className="team-profile-controls">
        <section className="settings-panel"><header><div><span>KİMLİK ÖNBAXIŞI</span><h2>Komanda təqdimatı</h2><p>Bu sahələr yalnız önbaxışı dəyişir. Profil məlumatlarının dərc edilməsi hazırda əlçatan deyil.</p></div></header><div className="settings-section">
          <div className="form-grid"><Input label="Komanda adı" value={draft.name} onChange={event => setDraft(value => ({ ...value, name: event.target.value }))} /><Input label="Qısa teq" value={draft.tag ?? ''} maxLength={12} onChange={event => setDraft(value => ({ ...value, tag: event.target.value }))} /></div>
          <div className="form-grid"><Input label="Ölkə" value={draft.country ?? ''} onChange={event => setDraft(value => ({ ...value, country: event.target.value }))} /><Input label="Qurulduğu tarix" type="date" value={draft.foundedAt?.slice(0, 10) ?? ''} onChange={event => setDraft(value => ({ ...value, foundedAt: event.target.value || undefined }))} /></div>
          <Textarea label="Komanda haqqında" value={draft.description ?? ''} rows={4} onChange={event => setDraft(value => ({ ...value, description: event.target.value }))} />
          <Input label="Banner təsviri" value={draft.bannerAlt ?? ''} onChange={event => setDraft(value => ({ ...value, bannerAlt: event.target.value }))} />
          <Button variant="secondary" onClick={() => setShowMedia(true)} disabled={showMedia} aria-expanded={showMedia}>{showMedia ? 'Media seçimləri açıqdır' : 'Logo və banneri önizlə'}</Button>
