...
export class Repository {
 ...
 async comparisons():Promise<TeamComparisonRecord[]> {
  ...
  return teams.filter(t=>t.approvalStatus==='approved'&&!t.archivedAt).map(team=>{
   ...
   return {teamId:team.id,teamName:team.name,matches:rows.length,finishes:rows.reduce((s,r)=>s+r.finishes,0),wwcd:rows.filter(r=>r.placement===1).length,podiums:rows.filter(r=>r.placement<=3).length,
